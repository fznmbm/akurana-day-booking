...
export default function AdminMealsPage() {
  ...
  const mealEnabled = useMealManagementEnabled();

  // Feature Gate: Check if meal management is enabled
  if (!mealEnabled) {
    return (
      <div
        style={{
      ...
      >
        ...
        >
          ...
          >
            Meal management is not enabled for this organization.
          </p>
          <button
  ...
  const exportCateringReport = () => {
    ...
    const rows = rsvps.map((rsvp) => {
      ...
      const age12plusBurger =
        rsvp.mealSelections?.filter(
          (m) =>
            m.ageCategory === "age12plus" && m.mealChoice === "burger-meal",
        ).length || 0;
